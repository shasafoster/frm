...
#     ois_fixings = ois_fixings.sort_values('date', ascending=True).reset_index(drop=True)
...
#     # Test single
#     d1 = pd.DatetimeIndex([pd.Timestamp(2022,10,4)]) # Coupon Accrual Start Date
#     d2 = pd.DatetimeIndex([pd.Timestamp(2023,10,3)]) # Coupon Accrual End Date

#     daily_compounded = calc_OIS_historical_coupon(d1, d2, ois_fixings, OISCouponCalcMethod.DailyCompounded, day_count_basis, value_date)
#     weighted_average = calc_OIS_historical_coupon(d1, d2, ois_fixings, OISCouponCalcMethod.WeightedAverage, day_count_basis, value_date)
#     simple_average = calc_OIS_historical_coupon(d1, d2, ois_fixings, OISCouponCalcMethod.SimpleAverage, day_count_basis, value_date)
...
# if __name__ == "__main__":    
#     test_calc_OIS_historical_coupon()
...
historical_fixings_df = pd.read_excel('C:/Users/shasa/Documents/frm_project/tests/term_structures/ois_publications.xlsx', sheet_name='sofr')
...
historical_fixings =   HistoricalSwapRateFixings(SwapType.OIS, historical_fixings_df,OISCouponCalcMethod.DailyCompounded)
