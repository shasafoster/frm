...
def test_fx_volatility_surface():
    ...
    vol_surface = FXVolatilitySurface(domestic_ccy=domestic_ccy,
                                      ...
                                      busdaycal=busdaycal,
                                      smile_interpolation_method=FXSmileInterpolationMethod.UNIVARIATE_SPLINE)
    ...
    for i, row in surf.vol_smile_pillar_df.iterrows():
        ...
        assert (np.abs(result['analytical_greeks'][delta_str][mask] - signed_delta[mask]) < 1e-8).all()
        assert (np.abs(result['analytical_greeks']['forward_delta'][~mask] - signed_delta[~mask]) < 1e-8).all()
