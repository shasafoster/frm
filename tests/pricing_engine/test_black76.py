...
def bachelier(F: [float, np.array],
    ...
    X = np.exp(-r*tau) * cp * \
        ( (F - K) * norm.cdf(cp * d) + σN * np.sqrt(tau) * norm.pdf(d) )
    results['price'] = X
...
def black76_ln_to_normal_vol_analytical(
    ...
    """
    Calculates the normal volatility from the Black76 log-normal volatility as detailed in Hagan's 2002 paper "Managing Smile Risk" (B.63, B.64).
    ...
    # Per B.63. in Hagan 2002.
    # https://quant.stackexchange.com/questions/58564/normal-vs-log-normal-implied-volatility
    # σN = σB * (F - K) * (1 - σB**2 * tau / 24) / np.log(F / K)

    # Per B.64 in Hagan 2002. More slightly more accurate than B.63.
    ln_F_K = np.log(F / K)
    σN = σB * np.sqrt(F*K) \
...
if __name__ == "__main__":
    ...
    # 28 June 2024, BBSW 3M

    test_shift_black76_vol()
    test_black76_ln_to_normal_vol()
    test_normal_vol_to_black76_ln()
...
#%%%
    ...
    F = 0.0447385
    tau = year_fraction(curve_date, start_date, day_count_basis)
    #fraction = (end_date - start_date).days / 365.25
    ...
    # Log-normal shift of 2
    market_sln2_vol = 0.1407
    atmf_sln2_vol = 0.1327
    # Log-normal shift of 1
    market_sln1_vol = 0.1677
    atmf_sln1_vol = 0.1569
    # Log-normal shift of 0
    market_sln0_vol = 0.2074
    atmf_sln0_vol = 0.1921

    X = black76(F=F, tau=tau, K=K, r=F, cp=1, vol=market_sln0_vol)
    print(X['price'] * 100 * 1e6)
    ...
    d2 = d1 - σ*np.sqrt(tau)
    px = np.exp(-r*tau) * cp * (F * norm.cdf(cp * d1) - K * norm.cdf(cp * d2))
    print('px', px, px * 100 * 1e6)
