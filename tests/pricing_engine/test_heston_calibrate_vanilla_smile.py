...
from frm.pricing_engine.heston import heston_calibrate_vanilla_smile, VALID_HESTON_PRICING_METHODS
import numpy as np
import matplotlib.pyplot as plt


def test_heston_calibrate_vanilla_smile():
    # Initialize
    delta_of_quotes_for_plot = np.array([0.1, 0.25, 0.5, 0.75, 0.9])
    delta_of_quotes = np.array([0.1, 0.25, 0.5, -0.25, -0.1])
    S0 = 1.2779
    tau = np.array([7 / 365, 1 / 12, 3 / 12, 6 / 12, 1, 2])
    r = np.array([0.31100, 0.32875, 0.49781, 0.70075, 1.08, 1.08]) * 0.01
    q = np.array([0.58, 0.631, 0.884, 1.131, 1.399, 1.399]) * 0.01
    ...
    if __name__ == "__main__":    
        ...
        T, D = np.meshgrid(tau, delta_of_quotes_for_plot)
        vols = np.array(volatility_surface).T  # Transpose to match dimensions
    ...
    for i, volatility_smile in enumerate(volatility_surface):
        
        var0, vv, kappa, theta, rho, lambda_, IV, SSE = heston_calibrate_vanilla_smile(
            volatility_quotes=volatility_smile, delta_of_quotes=delta_of_quotes, S0=S0, r=r[i], q=q[i], tau=tau[i],
            cp=cp, delta_convention=delta_convention, pricing_method=pricing_method)
        ...
        if __name__ == "__main__":
            ...
            # Plot 
            plt.figure(i+1)
            plt.plot(delta_of_quotes_for_plot * 100, volatility_smile * 100, 'ko-', linewidth=1)
            plt.plot(delta_of_quotes_for_plot * 100, IV, 'bs-', linewidth=1)
            plt.plot(delta_of_quotes_for_plot * 100, MATLAB_IV_SSE[i][:-1], 'rs--', linewidth=1)
            plt.legend([f'{tenors[i]} smile', f'frm - Heston fit via {pricing_method}', 'MATLAB - Heston fit'], loc='upper left')
            plt.xlabel('Delta [%]')
            plt.ylabel('Implied volatility [%]')
            plt.xticks(delta_of_quotes_for_plot * 100)
            plt.title('Comparison b/t market vols and calculated implied vols')
            plt.show()
    

if __name__ == "__main__":
    test_heston_calibrate_vanilla_smile()