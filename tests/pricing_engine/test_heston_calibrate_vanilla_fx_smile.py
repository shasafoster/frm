...
from frm.pricing_engine.heston_garman_kohlhagen import heston_calibrate_vanilla_fx_smile, VALID_HESTON_PRICING_METHODS
import numpy as np
import matplotlib.pyplot as plt


def test_heston_calibrate_vanilla_fx_smile():   
    # Initialize
    ...
    for i, σ_market in enumerate(σ_market_set):
        ...
        var0, vv, kappa, theta, rho, lambda_, IV, SSE = heston_calibrate_vanilla_fx_smile(Δ, Δ_convention, σ_market, S0, r_f[i], r_d[i], tau[i], cp, pricing_method=pricing_method)        
...
if __name__ == "__main__":
    test_heston_calibrate_vanilla_fx_smile()