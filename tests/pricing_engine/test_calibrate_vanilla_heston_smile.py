...
from frm.pricing_engine.heston import calibrate_vanilla_heston_smile, VALID_HESTON_PRICING_METHODS
import numpy as np
import matplotlib.pyplot as plt


def test_calibrate_vanilla_heston_smile():
    # Initialize
    Δ_for_plt = np.array([0.1, 0.25, 0.5, 0.75, 0.9]) # forward deltas
    
    Δ = np.array([0.1, 0.25, 0.5, -0.25, -0.1])
    r_ = np.array([0.31100, 0.32875, 0.49781, 0.70075, 1.08, 1.08]) * 0.01
    q_ = np.array([0.58, 0.631, 0.884, 1.131, 1.399, 1.399]) * 0.01
    ...
    for i, σ_market in enumerate(σ_market_set):
        ...
        var0, vv, kappa, theta, rho, lambda_, IV, SSE = calibrate_vanilla_heston_smile(Δ, Δ_convention, σ_market, S0, q_[i], r_[i], tau[i], cp, pricing_method=pricing_method)
...
if __name__ == "__main__":
    test_calibrate_vanilla_heston_smile()