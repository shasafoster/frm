...
def simulate_gbm_path(initial_px: np.array,
                      period_drift: np.array, 
                      period_forward_volatility: np.array,
                      period_length: np.array, 
                      rand_nbs: np.array):
    """
    ...
    period_drift (np.array): An array of drift rates for each period.
    period_forward_volatility (np.array): An array of volatilities for each period.
    period_length (np.array): An array representing the length of each period.
    rand_nbs (np.array): A 2D array of random numbers, where each row corresponds to a simulation
                         path and each column to a period.
    ...
    assert period_length.shape == period_drift.shape
    assert period_length.shape == period_forward_volatility.shape
    assert period_length.shape[0] == rand_nbs.shape[1]
    
    nb_periods = len(period_length)
    
    x = np.zeros((rand_nbs.shape[0],nb_periods+1))
    x[:,0] = initial_px
    
    for i in range(0, nb_periods):
        x[:,i+1] = x[:,i] * np.exp((period_drift[i] - 0.5 * period_forward_volatility[i] ** 2) * period_length[i] + period_forward_volatility[i] * np.sqrt(period_length[i]) * rand_nbs[:,i])
...
if __name__ == "__main__":
    ...
    period_drift = np.array([0.95*0.01, 0.55*0.01, -0.02*0.01, -0.34*0.01])
    period_forward_volatility = np.array([9.73*0.01, 10.10*0.01, 9.75*0.01, 10.97*0.01])
    period_length = np.array([0.512328767, 0.498630137, 0.501369863, 0.498630137])
    
    rand_nbs = generate_rand_nbs(nb_of_periods=len(period_drift),
                                 nb_of_simulations=1000 * 1000)
