# -*- coding: utf-8 -*-
"""
https://github.com/ted-love/SLV_Calibration/tree/38cde90a724d9c97574e03cb0d11412152df7e04/tools
"""
...
# S_0 = S0
# v, Uk = heston_cosine_method_SLV(S_0, K[i], tau, N, L, rd, rf, v_bar, v0, sigma, rho, kappa, cp[i])
# print('SLV (v, Uk):', v, sum(Uk))
...
def cumulants_truncation(L,T,r,q,v_bar,v0,sigma,rho,kappa):
    ...
    #print("SLV: c1,c2", c_1, c_2)
    
    return a,b


def heston_cosine_method_SLV(S,K,T,N,L,r,q,v_bar,v0,sigma,rho,kappa,flag):
    """
    ...
    #print('SLV method: (a,b)',a,b)
 
    k = np.linspace(0,N-1,N).reshape(N,1)
    omega = k*np.pi / (b-a)
    #print("SLV omega: ", sum(omega))
    
    character_func = charact_func(omega, r, q, rho, sigma, kappa, v0, v_bar, T)
    #print('SLV method: SUM(chf)', sum(character_func))
    ...
    if np.size(K) > 1:
        ...
        if flag == 1:
            ...
            return v + S*np.exp(-q*T) - K*np.exp(-r*T), Uk
