...
class Bond(object):
    def __init__(self, theta, kappa, sigma, r0=0.):
        ...
        kappa is the speed of convergence (i.e alpha)
    ...
    def StochasticPrice(self, VectorRates, VectorTime):  
        ...
        for i in range(No_Sim):    
            ...
            price[i] = np.exp(-(scipy.integrate.simpson(y=Rates , x=VectorTime)))
    ...
    def FutureZCB(self,M, I, T_0, T_M, Bond):  
        ...
        for i in range(No_Sim):    
            ...
            price[i]   = np.exp(-(scipy.integrate.simpson(y=Rates , x=VectorTime)))*Payoff[i]
            priceUntil_FO[i]  = np.exp(-(scipy.integrate.simpson(y=Rates, x=VectorTime)))
