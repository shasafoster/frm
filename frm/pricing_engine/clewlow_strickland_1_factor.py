...
from frm.pricing_engine.monte_carlo_generic import MAX_RANDOM_NUMBERS

import time
from prettytable import PrettyTable

def clewlow_strickland_1_factor(forward_curve, nb_simulations, segments_per_day, T, alpha, sigma, K=None):
    
    def clewlow_strickland_1_factor_simulate(ln_spot_px, ln_forward_curve_interp, term1, term3, σ, a, rand_nbs):
    ...
    f = interp1d(forward_curve_df['days'], forward_curve_df['forward_px'], kind='linear') # Interpolate the forward curve based on the steps 
    forward_curve_interp = f(tau)
    index_spot = np.arange(T+1) * segments_per_day
    ln_forward_curve_interp = np.log(forward_curve_interp)
    
    
    # Try to run the discretised simulations, store the indexed only, then return the indexed 
    # With a max of 100,000,000 random numbers that allows 1000 steps and 100,000 simulations which is pretty good.
    
    if nb_simulations > int(MAX_RANDOM_NUMBERS / nb_steps):
        
    
    
    ln_spot_px = np.zeros((nb_steps+1, nb_simulations))
    ...
    if K is None:
        # If no strike, specified, just simulate and return spot prices
        rand_nbs = generate_rand_nbs(nb_steps=nb_steps, nb_rand_vars=1, nb_simulations=nb_simulations, flag_apply_antithetic_variates=False, random_seed=0)
        ln_spot_px = clewlow_strickland_1_factor_simulate(ln_spot_px, ln_forward_curve_interp, term1, term3, σ, a, rand_nbs)
        spot_px = np.exp(ln_spot_px)
        return spot_px[index_spot,:]
    else:
        # If strike is specified, value call/put options 
        K = np.atleast_1d(K)
        
        if np.atleast_1d(K).shape == (1,):
            K = K * np.ones(shape=tau.shape)
  
        i = 0
        simulations_ran = 0
        max_simulations_per_while_loop = min(nb_simulations, int(MAX_RANDOM_NUMBERS / nb_steps))
        ...
        european_call_px_monte_carlo_list = []
        european_put_px_monte_carlo_list = []
        
        while simulations_ran < nb_simulations:
            rand_nbs = generate_rand_nbs(nb_steps=nb_steps, nb_rand_vars=1, nb_simulations=max_simulations_per_while_loop, flag_apply_antithetic_variates=False, random_seed=i)
            ln_spot_px = clewlow_strickland_1_factor_simulate(ln_spot_px, ln_forward_curve_interp, term1, term3, σ, a, rand_nbs)
            spot_px = np.exp(ln_spot_px)
        
            european_call_px_monte_carlo_list.append(np.maximum(0, spot_px - K[:, np.newaxis]).mean(axis=1))
            european_put_px_monte_carlo_list.append(np.maximum(0, K[:, np.newaxis] - spot_px).mean(axis=1))
        
            i += 1
            simulations_ran += max_simulations_per_while_loop
        
        european_call_px_monte_carlo = np.maximum(0, spot_px - K[:, np.newaxis])
        european_put_px_monte_carlo = np.maximum(0, K[:, np.newaxis] - spot_px)
        
        # Helpers for analytical pricing of European options 
        w = 0.5 * σ**2 * (1 - np.exp(-2*a*tau)) / a
        h = np.zeros(shape=w.shape)
        h[1:] = (ln_forward_curve_interp[1:] - np.log(K[1:]) + 0.5 * w[1:]) / np.sqrt(w[1:])
        
        # (Undiscounted) Analytical European call and put option prices for each step
        european_call_px_analytical = forward_curve_interp * norm.cdf(h) - K * norm.cdf(h - np.sqrt(w))
        european_put_px_analytical = K * norm.cdf(-h + np.sqrt(w)) - forward_curve_interp * norm.cdf(-h)
        
        df = pd.DataFrame(data=tau, columns=['tau'])
        df['forward_price'] = forward_curve_interp
        df['term1'] = term1
        df['term3'] = term3
        df['K'] = K
        df['w'] = w
        df['h'] = h
        df['european_call_px_analytical'] = european_call_px_analytical
        df['european_put_px_analytical'] = european_put_px_analytical
        df['european_call_px_monte_carlo'] = european_call_px_monte_carlo.mean(axis=1)
        df['european_put_px_monte_carlo'] = european_put_px_monte_carlo.mean(axis=1)
...
# Ideal function is:
# Provide a forward curve
# Strike curve
# desired granualry of output
# alpha, vol
# the function determines the max granularity it can apply to give you say, 10,000 or 20,000 simulations
# or whatever simulation number you input
...
forward_curve_df = pd.DataFrame(forward_curve_arr, columns=['days','forward_px'])
...
K = forward_curve_df.at[0,'forward_px'] # Strike set to current price 

for i, dt in enumerate([0.25, 1, 0.0625, 0.03125]):
    ...
    nb_simulations_list = [50e3, 2e3, 4e3, 10e3, 15e3, 20e3, 100e3]
    
    for j, nb_simulations in enumerate(nb_simulations_list):
        df = clewlow_strickland_1_factor(forward_curve_df, nb_simulations, segments_per_day, T, alpha, sigma, K=K)
    
        call_px_analytical.append(df['european_call_px_analytical'].values[1:].mean())
        call_px_monte_carlo.append(df['european_call_px_monte_carlo'].values[1:].mean())
        call_px_error.append(100*(call_px_analytical[j] - call_px_monte_carlo[j]))
    
        put_px_analytical.append(df['european_put_px_analytical'].values[1:].mean())
        put_px_monte_carlo.append(df['european_put_px_monte_carlo'].values[1:].mean())
        put_px_error.append(100*(put_px_analytical[j] - put_px_monte_carlo[j]))
    
    table = PrettyTable()
    print('')
    print(f'Case {i}, segments per day={segments_per_day}')
    table.add_column("Sims", nb_simulations_list)
    table.add_column("Call Analytical £/MWh", [round(v,4) for v in call_px_analytical])
    table.add_column("Call Monte £/MWh", [round(v,4) for v in call_px_monte_carlo])
    table.add_column("Call Error (%)", [round(v,2) for v in call_px_error])
    table.add_column("Put Analytical £/MWh", [round(v,4) for v in put_px_analytical])
    table.add_column("Put Monte £/MWh", [round(v,4) for v in put_px_monte_carlo])
    table.add_column("Put Error (%)", [round(v,2) for v in put_px_error])
    print(table)
...
#     n = 730
#     print('------------------------------------')
#     print(f'Spot price for n={n}:')
#     print(' - Forward price:', round(forward_curve_df.loc[forward_curve_df['days']==n,'forward_px'].values[0],dp))
#     print(' - Monte carlo average price:', round(spot_px_monte_carlo[n,:].mean(),dp))    
#     print('')
#     print('------------------------------------')
#     print(f'European call option price for n={n}, K={round(K,dp)}:')
#     print(' - Analytical option price:', round(european_call_px[n],dp))
#     print(' - Monte carlo average price:', round(call_px_monte_carlo[n,:].mean(),dp))   
#     print('')
#     print('------------------------------------')
#     print(f'European put price for n={n}, K={round(K,dp)}:')
#     print(' - Analytical option price:', round(eu_put_px[n],dp))
#     print(' - Monte carlo average price:', round(put_px_monte_carlo[n,:].mean(),dp))    
