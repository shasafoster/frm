...
@dataclass
class HistoricalSwapRateFixingsBase(ABC):
    ...
    @abstractmethod
    def get_historical_coupon_rate(self):
        pass  # Define abstract methods that derived classes must implement



@dataclass
class HistoricalTermSwapRateFixings(HistoricalSwapRateFixingsBase):
    def __post_init__(self):
        super().__post_init__()
    ...
    def get_historical_coupon_rate(self, fixing_dates: pd.DatetimeIndex):
...
@dataclass
class HistoricalOISSwapRateFixings(HistoricalSwapRateFixingsBase):
    ois_coupon_calc_method: OISCouponCalcMethod
    ...
    def get_historical_coupon_rate(
        self,
        ...
        for i, (observation_start, observation_end) in enumerate(zip(observations_start, observations_end)):
            if observation_end >= value_date:
                ...
                historical_coupon_rates[i] = np.nan
            else:
