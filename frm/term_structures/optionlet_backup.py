...
class Optionlet:
    ...
    def __post_init__(self):
        ...
        if solve_2Y_sabr_params:
            ...
            optionlet_pxs = np.full(shape=(len(self.optionlet_df[mask_prior_quotes]), len(self.quote_columns)), fill_value=np.nan)
            for i,row in self.optionlet_df.loc[mask_prior_quotes, :].iterrows():
            ...
            for idx, (i, row) in enumerate(self.optionlet_df.loc[mask_current_quotes, :].iterrows()):
    ...
    def _sse_2Y_vol_n_atm(self,
        ...
        last_optionlet_for_prior_quote_nb = pd.Series(mask_prior)[::-1].idxmax()
        last_optionlet_for_this_quote_nb = pd.Series(mask_current)[::-1].idxmax()
    ...
    def _solve_vol_n_atm(self, quote_nb):
        ...
        diff_to_solve_to_zero = self.capfloor_pxs.loc[quote_nb, 'atm'] - sum(optionlet_px)

        # Solve for the 2Y atm vol by finding the value of caplets that equal's
        res = scipy.optimize.minimize(
            fun=lambda param: self._sse_2Y_vol_n_atm(param=param, quote_nb=quote_nb, target=diff_to_solve_to_zero, normal_vol_prior_pillar=normal_vol_prior_pillar, sse_multiplier=1e6),
            x0=0.01,
            bounds=[(0.0001, None)])
        ...
        X = [self.optionlet_df.loc[last_optionlet_for_prior_quote_nb, 'expiry_years'],
             self.optionlet_df.loc[last_optionlet_for_this_quote_nb, 'expiry_years']]
        Y = [self.optionlet_df.loc[last_optionlet_for_prior_quote_nb, 'vol_n_atm'], vol_n_atm_pillar]
