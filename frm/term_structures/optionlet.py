...
class Optionlet:
    ...
    def get_sabr_params(self, expiry_date: pd.Timestamp):
        # TODO make these functions in the script 1st so don't have to reload them each time

        lower_index = pd.Series(self.term_structure['period_start'] <= expiry_date)[::-1].idxmax()
        upper_index = pd.Series(self.term_structure['period_start'] >= expiry_date)[::-1].idxmax()

        # Linearly interpolate the SABR parameters between the prior and current pillar points.
        X = [self.term_structure.loc[lower_index, 'expiry_years'],
             self.term_structure.loc[upper_index, 'expiry_years']]

        Y = [
            self.term_structure.loc[lower_index, 'beta','rho','volvol','vol_n_atm'],
            self.term_structure.loc[upper_index, 'beta','rho','volvol','vol_n_atm'],
        ]

        x = year_fraction(self.term_structure.loc[lower_index, 'period_start'], expiry_date, self.day_count_basis)
        ...
        # alpha = solve_alpha_from_sln_vol(
        #     tau=year_fraction(self.curve_date, expiry_date, self.day_count_basis),
        #     F=self.zero_curve.get_forward_rate(expiry_date),
        #     beta=beta,
        #     rho=rho,
        #     volvol=volvol,
        #     vol_sln_atm=vol_n_atm,
        #     ln_shift=self.ln_shift
        # )

    def get_vol_sln(self, expiry_date: pd.Timestamp, strike: float):

        # beta, rho, volvol, vol_n_atm = self.get_sabr_params(expiry_date=expiry_date)
        #
        # vol_sln = calc_sln_vol_for_strike(
        #     tau=year_fraction(self.curve_date, expiry_date, self.day_count_basis),
        #     F=self.zero_curve.get_forward_rate(expiry_date),
    ...
    def get_vol_n(self, expiry_date: pd.Timestamp, strike: float):
        pass


    def price_capfloor(self, effective_date: pd.Timestamp, termination_date: pd.Timestamp, strike: float, cp: str):
        # Do we bother to add all the stub params in?
        # Or have a "create schedule" function that does this?
        # Then a price function that expects a schedule format
        pass
