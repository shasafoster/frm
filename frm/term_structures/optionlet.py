...
class Optionlet:
    ...
    def __post_init__(self):
            ...
            for quote in self.quote_columns:
                ...
                if solve_method == 1:
                    ...
                    res = scipy.optimize.minimize(
                        fun=lambda param: self._capfloor_black76_ln_to_normal_vol_sse_helper(param=param, quote_nb=quote_nb, quote=quote, target=target),
                        x0=x0,
                        bounds=[(x0*0.8, x0*1.25)],
                        tol=1e-4 # Prices are within 0.01% - i.e. 10,000 +/- 1
                    )
        ...
        for quote_nb in range(len(self.capfloor_vol_quote_df)):
            ...
            if 'beta' in self.capfloor_vol_quote_df.columns and not pd.isnull(self.capfloor_vol_quote_df.loc[quote_nb, 'beta']):
                beta_overide = self.capfloor_vol_quote_df.loc[quote_nb, 'beta']
            else:
                beta_overide = None

            beta_overide = betas[quote_nb]
    ...
    def _solve_vol_n_atm(self, quote_nb):
        ...
        if quote_nb == 0:
            ...
            res = scipy.optimize.minimize(
                fun=lambda param: error_function(self=self, param=param),
                x0=0.01,
                bounds=[(0.0001, None)],
    ...
    def _solve_sabr_params(self, quote_nb, beta_overide=None):
        ...
        # Store the optionlets SABR params in 'optionlet_df'
        for idx, (i, row) in enumerate(self.optionlet_df.loc[mask_current, :].iterrows()):
