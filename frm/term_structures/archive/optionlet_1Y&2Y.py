...
class Optionlet:
    ...
    def __post_init__(self):
        ...
        for quote_nb, _ in self.capfloor_vol_quote_df.iterrows():
            ...
            for i, row_optionlet in self.optionlet_df[mask].iterrows():
                black76_optionlet_pxs = black76(
                    ...
                    K=K,
                    vol_sln=vol_sln,
                    ln_shift=self.ln_shift)['price'] * row_optionlet['annuity_factor']
            ...
            for quote in self.quote_columns:
                ...
                if solve_method == 1:
                    ...
                    res = scipy.optimize.minimize(
                        fun=lambda param: self._capfloor_black76_ln_to_normal_vol(param=param, quote_nb=quote_nb, quote=quote, target=target, sse_multiplier=sse_multiplier),
                        x0=x0,
        ...
        if solve_alpha_flag:
            ...
            res = scipy.optimize.minimize(
                fun=lambda param: self._1st_pillar_vol_sse(param, quote_nb=quote_nb, sse_multiplier=sse_multiplier, beta_overide=beta_overide),
                x0=x0,
                bounds=bounds)
        ...
        def sse_2Y_atm_vol(param,
            ...
            Y = [optionlet_normal_vol[-1], param.item()]
            ...
            return sse_multiplier * (target - optionlet_px) ** 2


        res = scipy.optimize.minimize(
            fun=lambda param: sse_2Y_atm_vol(param=param, quote_nb=quote_nb, target=diff_to_solve_to_zero, sse_multiplier=1e6),
            x0=0.1,
            bounds=[(0.0001, None)])
        ...
        # # Solve the
        # for i, row in self.optionlet_df.loc[mask, :].iterrows():
        #     self.optionlet_df.loc[i, 'vol_sln_atm'] = normal_vol_to_black76_ln(
        #         F=row['F'],
        #         tau=row['expiry_years'],
        #         K=row['F'],
        #         vol_n=row['vol_n_atm'],
     ...
     #    last_optionlet_for_this_quote_nb = pd.Series(self.optionlet_df['quote_nb'] == quote_nb)[::-1].idxmax()
     #
     #    # For the first pillar point, use a flat beta, rho, volvol over the tenor.
     #    beta, alpha, rho, volvol = (beta_overide, *param) if beta_overide is not None else param
     #
     #    # Solve the Black76 volatility for the at-the-money optionlet from the normal volatility.
     #
     #
    ...
    def _1st_pillar_vol_sse(self, param, quote_nb, sse_multiplier, beta_overide=None):
        if beta_overide is None:
            beta_overide, rho, volvol = param
        else:
            rho, volvol = param
            beta = beta_overide
    ...
    def _capfloor_black76_ln_to_normal_vol(self,
                        param: np.array,
                        quote_nb: int,
                        quote: str,
                        target: float,
        ...
        bachelier_optionlet_pxs = bachelier(
            ...
            K=self.strikes_df.loc[quote_nb, quote],
            vol_n=param)['price'] * self.optionlet_df[mask]['annuity_factor']
